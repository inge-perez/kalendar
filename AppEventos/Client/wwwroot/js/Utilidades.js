...
//Funcion para desloguear si el usuario no hace nada en la pagina
function timerInactivo(dotnetHelper) {
    ...
    function resetTimer() {
        clearTimeout(timer);
        timer = setTimeout(logout, 600000); //10 minutos
    }
...
function Reload() {
	$('.select2').select2({
		...
		DotNet.invokeMethodAsync('AppEventos.Client', 'UpdateModel', selectedVal);
